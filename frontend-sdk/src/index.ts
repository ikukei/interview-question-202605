...
export function createFeatureClient(options: FeatureClientOptions): FeatureClient {
  ...
  async function evaluate(flagKey: string, context: FeatureContext): Promise<FeatureEvaluation> {
    const response = await fetch(`${baseUrl}/api/v1/evaluations/flags/${encodeURIComponent(flagKey)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ appKey: options.appKey, environment: options.environment, context })
    });
    if (!response.ok) {
  ...
  async function listFlagKeys(): Promise<string[]> {
    const response = await fetch(
      `${baseUrl}/api/v1/flags?appKey=${encodeURIComponent(options.appKey)}&environment=${encodeURIComponent(options.environment)}`,
  ...
  async function evaluateAll(context: FeatureContext): Promise<FeatureEvaluation[]> {
    const flagKeys = await listFlagKeys();
    if (flagKeys.length === 0) return [];
    const response = await fetch(`${baseUrl}/api/v1/evaluations:batch`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ appKey: options.appKey, environment: options.environment, flagKeys, context })
    });
    if (!response.ok) {
      const message = await response.text();
  ...
  return {
    ...
    async boolVariation(flagKey: string, context: FeatureContext, defaultValue = false): Promise<boolean> {
      try {
        const result = await evaluate(flagKey, context);
